...
The directory specified in `bbdb-sound-directory' is searched for the files
touchtone.*\\.\\(wav\\|au\\) as named in `bbdb-sound-files'.
They are stored ins `sound-alist' as touchtone0 to touchtone11."
  (interactive)
  (let ((files (directory-files
        ...
        (nr 0))
    (if (not files)
        (error "No touchtone files found in `bbdb-sound-directory'!"))
    (while files
