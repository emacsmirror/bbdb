...
dnl Last Revision: $Date: 2013/04/13 15:22:16 $

dnl Copyright (C) 2000-2001 Didier Verna <didier@xemacs.org>
dnl Copyright (C) 2011-2013 Roland Winkler <winkler@gnu.org>
...
AC_DEFUN([BBDB_WITH_VM],
  [ AC_ARG_WITH([vm-dir],
    ...
    if test "x$with_vm_dir" != "x"; then
       ...
       dnl convert path to absolute and canonicalize it.
       VMDIR=`${EMACS_PROG} -batch --quick -eval "(message \"%s\" (expand-file-name \"${with_vm_dir}\"))" 2>&1`
       BBDB_VM=`${EMACS_PROG} -batch --quick --directory=${VMDIR} -eval "(if (or (locate-library \"vm-autoload\") (locate-library \"vm-autoloads\")) (message \"vm\"))" 2>&1`
